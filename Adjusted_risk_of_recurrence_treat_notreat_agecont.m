function [] = Adjusted_risk_of_recurrence_treat_notreat_agecont(UTI_cases,params)

number_of_drugs = params.number_drugs;
X_age = UTI_cases.Demog.Age;
X_age(:,6) = [];

%X_demog = [UTI_cases.Demog.Agenum, UTI_cases.Demog.Agenum.^2, UTI_cases.Demog.Gender, UTI_cases.Demog.SeniorHouse UTI_cases.Demog.Preg ];
X_demog = [X_age, UTI_cases.Demog.Gender, UTI_cases.Demog.Preg ];


%% OR specifically StoR failure given treated vs untreated
clear y_SSR p_SSR ciu_SSR cil_SSR se_SSR 
%exclude treatment fails less than 5 days
figure;
set(gcf,'color','w')
clf; set(gcf,'name','Fig. S4 Adjusted risk of recurrence and emergence of resistance with and without antibiotic treatment');
