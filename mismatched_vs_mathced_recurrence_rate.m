function []= mismatched_vs_mathced_recurrence_rate(FTRSp,params)
% mismatched vs matched recurrence rate
...
clear  all_sensitive all_sensitive_purchased sensitive_purchased_fails total_prch
...
for drug = 1:params.number_drugs  
   ...
   all_reistant = (ismember(FTRSp.SMP_Res(:,drug), params.resistant_group))  & FTRSp.hasdiag ;
   ...
   all_resistant_purchased(drug) =  nnz(all_reistant & FTRSp.PCR_sameday(:,drug));
   resistant_purchased_fails(drug) = nnz(all_reistant & FTRSp.PCR_sameday(:,drug) & FTRSp.treatfailure);
   resistant_purchased_nextresfails(drug) = nnz(all_reistant & FTRSp.PCR_sameday(:,drug) & FTRSp.treatfailure & all_nextres);
   
   resistant_purchased_fails_RS(drug) = nnz(all_reistant & FTRSp.PCR_sameday(:,drug) & FTRSp.treatfailure & all_sensitive_nextres);
   resistant_purchased_fails_RR(drug) = nnz(all_reistant & FTRSp.PCR_sameday(:,drug) & FTRSp.treatfailure & all_resistant_nextres);
end
