function [] = correct_treatment_make_figs_regression_diag_bar_PURCHandRES(UTI_cases,params)
number_of_drugs = 7;

X_age = UTI_cases.Demog.Age;
X_age(:,6) = [];
X_demog = [X_age, UTI_cases.Demog.Gender, UTI_cases.Demog.Preg  ];
features = {'Age 0-9', 'Age 10-19', 'Age 20-29', 'Age 30-39', 'Age 40-49', 'Age 60-69', 'Age 70-79', 'Age 80-89', 'Age 90-100', 'Gender', 'Ret Home', 'Pregnancy', 'Pyelo', 'diabetes','CKD', 'CATH', 'Prev Resistance'};
 
num_prev_purch = UTI_cases.num_prev_purch;
...
% seall = zeros(size(X_demog,2) +2, number_of_drugs_7);
% yall = zeros(size(X_demog,2) +2, number_of_drugs_7);
% pall = zeros(size(X_demog,2) +2, number_of_drugs_7);
...
%SMP_to_use =  find(UTI_cases.any_SRmeasurement_1year(:,drug)>0 & (UTI_cases.SMP_Res(:,drug)== 1 | UTI_cases.SMP_Res(:,drug)== 2) & UTI_cases.next_res(:, drug) ~=0  );
...
%SMP_to_use =  find( (UTI_cases.SMP_Res(:,drug)== 1 | UTI_cases.SMP_Res(:,drug)== 2) & UTI_cases.next_res(:, drug) ~=0  & UTI_cases.hasdiag);
...
% age_coef = c.coef(2:10);
% age_p = c.p(2:10);
% age_se = c.se(2:10);
% [M,I] = max(age_coef);
% y = zeros(size(c.coef,1)-9,1);
% p = zeros(size(c.coef,1)-9,1);
% se = zeros(size(c.coef,1)-9,1);
% y(2:end) = (c.coef(11:end));
% y(1) = (age_coef(I));
% p(2:end) = c.p(11:end);
% p(1) = age_p(I);
% se(2:end) = c.se(11:end);
% se(1) = age_se(I);
