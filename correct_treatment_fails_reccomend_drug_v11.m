function [] = correct_treatment_fails_reccomend_drug_v11(UTI_cases,params)
...
for drug = 1:number_of_drugs
%subplot(number_of_drugs,1,drug)
%hist(P_nonans(P_nonans(:,drug)<1,drug),bins)
%xlim([0 0.3])
...
%hatch(hd3(ii),[45,3,1],'w')
%hatch(hd4(ii),[45,3,1],'k')
...
%% just cipro sensitive w/ past reisitance
real_prch = UTI_cases.PCR_sameday(withPdata_index, 1:number_of_drugs);
figure;
...
cip_sensitve = ismember(UTI_cases.SMP_Res(withPdata_index,drug),[1 2]);
...
% total = number_prescribed_reccomended./sum(number_prescribed_reccomended,2)
% ratio_cost(1,:) = total(2,:)./total(1,:)
% ratio_cost(2,:) = total(3,:)./total(1,:)
% ratio_cost(ratio_cost<1) = -ratio_cost(ratio_cost<1)
...
%% regression for any failure (S-S & S-R) 
sen_res = [ 1 2 3];
